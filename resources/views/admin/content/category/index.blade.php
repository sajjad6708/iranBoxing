                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <label>
                                    <input id="{{ $postCategory->id }}-status" onchange="changeStatus({{ $postCategory->id }})" data-url="{{ route('admin.content.category.status', $postCategory->id) }}" type="checkbox" @if ($postCategory->status === 1)
                                    checked
                                    @endif>
                                </label>
                            </td>
                            ...
                            <td>
                                <label>
                                    <input id="{{$postCategory->id}}" onchange="ShowInMenu({{ $postCategory->id }})" data-url="{{ route('admin.content.category.showInMenu', $postCategory->id) }}" type="checkbox" @if ($postCategory->show_in_menu === 1)
                                    checked
                                    @endif>
                                </label>
                            </td>
        ...
        $.ajax({
            ...
            success : function(response){
                if(response.ShowInMenu){
                    if(response.checked){
                        element.prop('checked', true);
                        successToast(' نمایش منو با موفقیت فعال شد')
                    }
                    else{
                        element.prop('checked', false);
                        successToast('نمایش منو  با موفقیت غیر فعال شد')
                    }
                }
                else{
                    element.prop('checked', elementValue);
