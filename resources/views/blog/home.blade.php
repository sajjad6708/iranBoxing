...
<br><br><br><br>
        ...
        <div class="container">
          <div class="row">
            <h1>پر بازدید ترین پست ها:</h1>
            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3">
      
              @foreach ($mostVisiteds as $mostVisited)
           <div class="col">
    
                <div class="card shadow-sm">
                  <img class="bd-placeholder-img card-img-top" width="100%" height="225" src="{{asset($mostVisited->image)}}"/>
                  <div class="card-body">
                    <strong> {{$mostVisited->title}}</strong>
                    <p class="card-text"> {!! Str::limit($mostVisited->summary , 80)!!} </p>
                    <div class="d-flex justify-content-between align-items-center">
                      <div class="btn-group">
                        <a href="{{ route('show-post' , $mostVisited) }}"><button type="button" class="btn btn-sm btn-outline-secondary">مشاهده</button></a>
                        
                      </div>
                      <small class="text-muted">تاریخ انتشار : {{jalaliDate($mostVisited->created_at)}}</small>
                    </div>
                  </div>
                </div>
