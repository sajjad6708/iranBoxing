...
class LoginRegisterController extends Controller
{
    ...
    {
       return view('blog.auth.login-register');
    ...
    public function loginRegister(LoginRegisterRequest $request)
     {
        ...
        if(filter_var($inputs['id'] ,FILTER_VALIDATE_EMAIL))
        {
         ...
         elseif(preg_match('/^(\+98|98|0)9\d{9}$/', $inputs['id'])){
            ...
            return redirect()->route('blog.auth.login-register-form')->withErrors(['id' => $errorText]);
        }
        ...
        if(empty($user))
        {
         ...
         return redirect()->route('blog.auth.login-confirm-form', $token);
     ...
     public function loginConfirmForm( $token)
     {
        ...
        if(empty($otp))
        {
            return redirect()->route('blog.auth.login-register-form')->withErrors(['id' => 'آدرس وارد شده نامعتبر میباشد']);
        }
        else
        {
            return view('blog.auth.login-confirm' ,compact('otp' , 'token'));
        }
     ...
     public function loginConfirm($token , LoginRegisterRequest $request)
     {
        ...
        if(empty($otp))
        {
            return redirect()->route('blog.auth.login-register-form', $token)->withErrors(['id' => 'ادرس وارد شده نامعتبر است']);
        }

        if ($otp->otp_code !== $inputs['otp'])
        {
            return redirect()->route('blog.auth.login-confirm-form', $token)->withErrors(['id' => 'کد وارد شده صحیح نمیباشد']);
        };
